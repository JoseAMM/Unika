<?php
    ...
    $idCargo = $_GET['id'];
    $idCargo = filter_var($idCargo, FILTER_VALIDATE_INT);
    ...
    $queryCargo = "SELECT * FROM cargo WHERE idCargo = $idCargo";
    $resultadoCargo = mysqli_query($db, $queryCargo);
    $resultadoCargo = mysqli_fetch_assoc($resultadoCargo);
    $resultadoCargo = $resultadoCargo['Nombre_Cargo'];
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $queryEditarCargo = "UPDATE cargo SET Nombre_Cargo = '$cargo' WHERE idCargo = $idCargo";

        $resultadoEditarCargo = mysqli_query($db, $queryEditarCargo);

        if($resultadoEditarCargo) {
...
<header class="header__propiedades">
    ...
    <section class="main__menu--content">
        ...
        <form action="" method="POST">
            ...
            <label for="cargo">
                <span>Cargo</span>
                <input type="text"  value="<?php echo $resultadoCargo?>" id= "cargo" name="cargo" placeholder = "Cargo Name" required maxlength="50">
            </label>
