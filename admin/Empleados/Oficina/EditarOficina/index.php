<?php
    ...
    $idOficina = $_GET['id'];
    $idOficina = filter_var($idOficina, FILTER_VALIDATE_INT);
    ...
    $queryOficina = "SELECT * FROM oficina WHERE idOficina = $idOficina";
    $resultadoOficina = mysqli_query($db, $queryOficina);
    $resultadoOficina = mysqli_fetch_assoc($resultadoOficina);
    $resultadoNombreOficina = $resultadoOficina['Nombre_Oficina'];
    $resultadoDireccion = $resultadoOficina['Direccion_Oficina'];
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $queryEditarOficina = "UPDATE oficina SET Nombre_Oficina = '$oficina' WHERE idOficina = $idOficina";
        $queryEditarDireccion = "UPDATE oficina SET Direccion_Oficina = '$direccion' WHERE idOficina = $idOficina";

        $resultadoEditarOficina = mysqli_query($db, $queryEditarOficina);
        $resultadoEditarDireccion = mysqli_query($db, $queryEditarDireccion);

        if($resultadoEditarOficina) {
...
<header class="header__propiedades">
    ...
    <section class="main__menu--content">
        ...
        <form action="" method="POST">
            ...
            <label for="oficina">
                <span>Oficina</span>
                <input type="text"  value="<?php echo $resultadoNombreOficina?>" id= "oficina" name="oficina" placeholder = "Oficina Name" required maxlength="45">
            </label>

            <label for="direccion">
                <span>Dirección</span>
                <input type="text"  value="<?php echo $resultadoDireccion?>" id= "direccion" name="direccion" placeholder = "Direccion" required maxlength="100">
            </label>
