...
$nombreClienteDerechos = $consultaTablaTemporalDocumentosOficiales['NombreClienteDerechos'];
$cargoClienteDerechos = $consultaTablaTemporalDocumentosOficiales['CargoClienteDerechos'];
$nombreAsesorDerechos = $consultaTablaTemporalDocumentosOficiales['NombreAsesorDerechos'];
$cargoAsesorDerechos = $consultaTablaTemporalDocumentosOficiales['CargoAsesorDerechos'];

if ($consultaTablaTemporalDocumentosOficiales['FirmaRecibidoDerechos'] != NULL) {
    $firmaRecibidoDerechos = $consultaTablaTemporalDocumentosOficiales['FirmaRecibidoDerechos'];
} else {
    $firmaRecibidoDerechos = false;
}

cartaDerechos($nombreClienteDerechos, $cargoClienteDerechos, $nombreAsesorDerechos, $cargoAsesorDerechos, $idInmueble, $nombreDocumento, $firmaRecibidoDerechos, $nombreImagenSubida);
...
function cartaDerechos(
    ...
    $idInmueble,
    $nombreDocumento,
    $firmaRecibidoDerechos,
    $nombreImagenSubida
) {

    global $pdf;
    global $db;
    if ($firmaRecibidoDerechos != false) {
        $nombreImagenRecibido = '../../../../Canva/assets/Firmas/' . $firmaRecibidoDerechos;
        $nombreImagenEntrega = '../../../../Canva/assets/Firmas/' . $nombreImagenSubida;
        $queryInsertarEnTablaTemporalImagenRecibido = "UPDATE informaciontemporaldocumentosoficiales SET FirmaEntregaDerechos = '$nombreImagenSubida' WHERE idInmueble_InformacionTemporalDocumentosOficiales = $idInmueble AND NombreDocumento = '$nombreDocumento'";
        $consultaInsertarEnTablaTemporalImagenRecibido = mysqli_query($db, $queryInsertarEnTablaTemporalImagenRecibido);
    } else {
        $nombreImagenRecibido = '../../../../Canva/assets/Firmas/' . $nombreImagenSubida;
        $queryInsertarEnTablaTemporalImagenRecibido = "UPDATE informaciontemporaldocumentosoficiales SET FirmaRecibidoDerechos = '$nombreImagenSubida' WHERE idInmueble_InformacionTemporalDocumentosOficiales = $idInmueble AND NombreDocumento = '$nombreDocumento'";
        $consultaInsertarEnTablaTemporalImagenRecibido = mysqli_query($db, $queryInsertarEnTablaTemporalImagenRecibido);
    }
...
14. Elegir libremente al notario público para realizar el trámite de escrituración.'), 0, 'FJ', 1);
     ...
     * * Firma recibido
     */
    $pdf->Image($nombreImagenRecibido, 43, 145, 30, 30);
    /**
    ...
    if ($firmaRecibidoDerechos != false) {
         ...
         * * Firma recibido
         */
        $pdf->Image($nombreImagenEntrega, 131, 140, 30, 30);
    } else {
        /**
        ...
        $pdf->Link(119, 160, 46, 10, 'https://unikabienesraices.com/Canva/index.php?id=' . $idInmueble . '&document=' . $nombreDocumento);
        //$pdf->Link(119, 160, 46, 10, 'http://localhost:3000/Canva/index.php?id=' . $idInmueble . '&document=' . $nombreDocumento);
        $pdf->SetY(165);
        $pdf->SetX(121);
    ...
    $pdf->SetTitle('CartaDerechos.pdf');
    if ($firmaRecibidoDerechos != false) {
        if (file_exists($nombreImagenRecibido)) {
            unlink($nombreImagenRecibido);
        }
        if (file_exists($nombreImagenEntrega)) {
            unlink($nombreImagenEntrega);
        }
        ...
        $pdf->Output('F', '../DocumentosFirmados/CartaDerechos' . $idInmueble . '.pdf');
    }
