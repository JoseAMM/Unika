...
function pdf(
    ...
    class PDF extends FPDF
    {
        ...
        {
            // Logo
            // $this->Image('./Assets/Icon Amenities/logo.png', 75, 13, 50);
        }
        ...
        function Footer()
        {
            ...
            // Número de página
            $this->Cell(0, 10, utf8('Página ') . $this->PageNo(), 0, 0, 'C');
        }
    }
    $pdf = new PDF();
    $pdf->AddPage('L');
    ...
    $pdf->Write(1, utf8($operacion));
    $pdf->SetY(185);
    $pdf->SetX(227);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('Black', '', 25);
    $pdf->Write(1, '$' . number_format(utf8($precio)));
    $pdf->SetY(10);
    $pdf->SetX(135);
    $pdf->SetTextColor(255, 0, 0);
    $pdf->SetFont('Bold', '', 20);
    $pdf->Write(1, utf8($inmueble));
    $pdf->Image('./Assets/Icon Amenities/SVG/locacion.png', 270, 5, 20, 20);
    $pdf->SetY(20);
    $pdf->SetX(135);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('Bold', '', 16);
    $pdf->Write(1, utf8($colonia));
    $pdf->SetY(35);
    ...
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('Bold', '', 25);
    $pdf->Write(1, utf8('Características'));
    ...
    $pdf->Write(1, utf8('Superficie de Terreno'));

    $pdf->SetY(65);
    $pdf->SetX(30);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('Bold', '', 35);
    $pdf->Write(1, utf8($superficie_terreno) . ' m2');

    $pdf->SetY(89);
    $pdf->SetX(13);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('Regular', '', 9);
    $pdf->Write(1, utf8('Superficie de Construcción'));
    ...
    $pdf->Write(1, utf8($superficie_construccion) . ' m2');
    ...
    if ($habitaciones != 0) {
        ...
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Regular', '', 15);
        $pdf->Write(1, utf8($habitaciones));

        $pdf->SetY(88);
        $pdf->SetX(68);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Regular', '', 6);
        $pdf->Write(1, utf8('Habitaciónes :)'));
    ...
    if ($estacionamiento != 0) {
        ...
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Regular', '', 6);
        $pdf->Write(1, utf8('Estacionamiento'));
    }
    ...
    while ($row = mysqli_fetch_assoc($consultaAmenidades)) {
        ...
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Regular', '', 6);
        $pdf->Write(1, utf8($row['NombreAmenidades']));
