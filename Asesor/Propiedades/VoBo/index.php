...
if (isset($_GET['del'])) {
    ...
    while ($row = mysqli_fetch_assoc($resultadoBuscarFotosInmueble)) {
        if (file_exists('../../../admin/Propiedades/Imagenes/' . $row['NombreFotos'])) {
            unlink('../../../admin/Propiedades/Imagenes/' . $row['NombreFotos']);
        }
    }
    ...
    while ($row = mysqli_fetch_assoc($resultadoBuscarDocumentosInmueble)) {
        if (file_exists('../../../admin/Propiedades/Documentos/Documents/' . $row['idDocumentos'])) {
            unlink('../../../admin/Propiedades/Documentos/Documents/' . $row['idDocumentos']);
        }
    }
    ...
    while ($row = mysqli_fetch_assoc($resultadoBuscarDocumentosOficialesInmueble)) {
        if (file_exists('../../../admin/Propiedades/Ver/DocumentosFirmados/' . $idInmueble . $row['NombreDocumentosOficial'] . '.pdf')) {
            unlink('../../../admin/Propiedades/Ver/DocumentosFirmados/' . $idInmueble . $row['NombreDocumentosOficial'] . '.pdf');
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        ...
        INNER JOIN caracteristicas ON inmueble.idInmueble = caracteristicas.idInmueble_Caracteristicas
        INNER JOIN datos_basicos ON inmueble.idInmueble = datos_basicos.Inmueble_idInmueble
        INNER JOIN colonias ON datos_basicos.Colonias_idColonias = colonias.id WHERE ";
        ...
        foreach ($_POST as $key => $value) {
            ...
            if (count($_POST) > 1) {
                ...
                if ($key == "Superficie_Terreno" or $key == "Superficie_Construccion" or $key == "Habitaciones" or $key == "Puestos_Estacionamiento") {
                    $queryBuscar = $queryBuscar . "caracteristicas." . $key . " = " . $value . " AND VoBo = 0 AND FotoPortada IS NOT NULL";
                    unset($_POST[$key]);
                } else {
                    $queryBuscar = $queryBuscar . "inmueble." . $key . " = " . $value . " AND VoBo = 0 AND FotoPortada IS NOT NULL";
                    unset($_POST[$key]);
                }
            }
        }
                    ...
                    <section class="table__content">
                        <table>
                            <tr>
                                ...
                                <td><img class="content__imagen" src="../../../admin/Propiedades/Imagenes/<?php if ($row['FotoPortada'] == NULL) { echo "vacio.png";} else { echo $row['FotoPortada'];}?>" alt=""></td>
